...
async def _get_plan_by_id(session: AsyncSession, plan_id: uuid.UUID) -> Plan:
...
async def _ensure_default_basic_plan(session: AsyncSession) -> Plan:
    stmt = select(Plan).where(Plan.name == PlanTier.BASIC)
...
async def create_school(payload: SchoolCreate, session: AsyncSession = Depends(get_db)) -> School:
    ...
    if plan_id:
        ...
        plan = await _ensure_default_basic_plan(session)
...
async def get_school(school_id: uuid.UUID, session: AsyncSession = Depends(get_db)) -> School:
    result = await session.execute(
